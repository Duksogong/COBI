...
function ReviewDetailPage() {
  ...
  const [bookmarkIcon, setBookmarkIcon] = useState(<HiOutlineBookmark fontSize="40px" />);
  ...
  useEffect(() => {
    ...
    const icon = selectedBookmarks.find((bookmark) => bookmark.reviewId === reviewId)
  ...
  const onSetBookmark = () => {
    ...
    let funBool = selectedBookmarks.find((bookmark) => bookmark.reviewId === reviewId) ? false : true ;
    ...
    setBookmarkIcon(funBool ? <HiMiniBookmark size="40px" /> : <HiOutlineBookmark size="40px" />);
  }
    ...
    <div className="d-flex flex-column" style={{height: '100vh'}}>
      ...
      >
        ...
        >
          ...
          <div style={{position: 'absolute', top: '-12px', right: '25px', color:'#A9B388'}} 
            onClick={() => onSetBookmark()}>{bookmarkIcon}</div>
